...
if ($tmp_show_user_images) {
    try {
        $req = $bdd->prepare('SELECT * FROM image INNER JOIN user ON image.img_user_id = user.id_user WHERE image.img_user_id = ? AND image.img_album = ? ORDER BY image.img_update_dt DESC');
        $req->execute(array($tmp_user_id, htmlspecialchars($_GET['alb'])));
        if ($req->rowCount() == 0) {
            echo '<script type="text/javascript">document.getElementById("album-nb-pics").innerHTML = "0";</script>';
            echo '<p class="bg-danger text-center">No images found for this album</p>';
        }
        else {
            echo '<script type="text/javascript">document.getElementById("album-nb-pics").innerHTML = "'.$req->rowCount().'";</script>';
        ...
        if (isset($user_session) && $user_session->id == $user_get->id) echo '&nbsp;<button type="submit" class="btn btn-default btn-md btn-actions" data-toggle="tooltip" data-placement="top" title="Delete"><i class="fa fa-times fa-lg"></i></button>';
                          echo '</form>&nbsp;
